...
if(isset($_GET["id"])){
    ...
    $detalles = ModeloDetallePresupuesto::mdlObtenerDetalles($_GET["id"]);
...
<div class="container mt-5">
    <div class="row">
        <div class="col-12">
            <h2 class="mb-4">Editar Presupuesto #<?php echo $presupuesto['id_presupuesto']; ?></h2>
                                        ...
                                        <?php foreach ($clientes as $cliente): ?>
                                            <option value="<?php echo $cliente['id_cliente']; ?>" 
                                                ...
                                                foreach($vehiculos as $v) {
                                                    if($v['id_vehiculo'] == $presupuesto['id_vehiculo'] && $v['id_cliente'] == $cliente['id_cliente']) {
                                                        echo 'selected';
                                                        break;
                                                    }
                                                }
                                    ...
                                    <select class="form-select" id="id_vehiculo" name="id_vehiculo" required>
                                        ...
                                        <?php foreach ($vehiculos as $vehiculo): ?>
                                            <option value="<?php echo $vehiculo['id_vehiculo']; ?>" 
                                                <?php echo ($vehiculo['id_vehiculo'] == $presupuesto['id_vehiculo']) ? 'selected' : ''; ?>>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" class="form-control" id="fecha_validez" name="fecha_validez" 
                                           value="<?php echo date('Y-m-d\TH:i', strtotime($presupuesto['fecha_validez'])); ?>" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_presupuesto'])) {
                $actualizar = PresupuestoControlador::ctrActualizarPresupuesto();
                if($actualizar == "ok"){
                    echo '<script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Cargar datos existentes del presupuesto
    ...
    detallesExistentes.forEach(detalle => {
        ...
        // Si es servicio, agregar tipo_servicio
        if (detalle.tipo === 'servicio') {
            detalleFormateado.tipo_servicio = detalle.tipo_servicio || 'servicio_general';
        }
        
        detalles.push(detalleFormateado);
    });
